const db = require('./src/db-adapter.js');
async function test() {
  ...
  const batch = await getRecentHybridMedia.all(10, 0);
  console.log("Found rows:", batch.length);
  if (batch.length > 0) {
    console.log("First row kind:", batch[0].kind, "platform:", batch[0].platform);
  }
}
