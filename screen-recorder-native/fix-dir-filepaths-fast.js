...
async function run() {
  ...
  for (const row of res.rows) {
    ...
    if (stat.isDirectory()) {
      ...
      const files = fs.readdirSync(row.filepath).filter(f => {
        const ext = path.extname(f).toLowerCase();
        return ['.mp4', '.mov', '.m4v', '.webm', '.mkv', '.avi', '.ts', '.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp'].includes(ext);
      });
  ...
  console.log(`Fixed ${fixed} recent entries.`);
  pool.end();
}
